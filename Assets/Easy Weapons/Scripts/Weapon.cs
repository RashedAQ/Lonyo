...
public class Weapon : MonoBehaviour
{
    ...
    public bool playerWeapon = Gun.isHoldingObject;




    // Whether or not this is a player's weapon as opposed to an AI's weapon
    ...
    // Crosshairs
    public bool showCrosshair = Gun.isHoldingObject;                 // Whether or not the crosshair should be displayed
    ...
    void Update()
    {
        showCrosshair = Gun.isHoldingObject && !PauseMenu.isGamePaused ;



        playerWeapon = Gun.isHoldingObject ;
    ...
    void Fire()
    {
        ...
        for (int i = 0; i < shotPerRound; i++)
        {
            ...
            // The ray that will be used for this shot - now using camera position
            ...
            if (Physics.Raycast(ray, out hit, range))
            {
                ...
                // In Weapon.cs, inside the Fire() method:
                Debug.Log($"Hit object: {hit.collider.name}"); // Check what's being hit
                hit.collider.gameObject.SendMessageUpwards("ChangeHealth", -damage);
                // Damage
                // Inside the Fire() method, after the raycast:
                if (Physics.Raycast(ray, out hit, range))
                {
                    Debug.Log("Hit: " + hit.collider.name); // Check what's being hit
                    Debug.DrawLine(ray.origin, hit.point, Color.red, 2f); // Visual ray in Scene view

                    // Rest of the damage code...
                }
